...
class Controller(object):
    def _validate_environment(self, unit, request, environment_id):
        ...
        if environment is None:
            LOG.info(_LI('Environment <EnvId {0}> is not found').format(environment_id))
            raise exc.HTTPNotFound

        if environment.tenant_id != request.context.tenant:
            LOG.info(_LI('User is not authorized to access this tenant resources.'))
    ...
    def execute(self, request, environment_id, action_id, body):
        ...
        if env_status in (states.EnvironmentStatus.DEPLOYING,
            ...
            LOG.info(_LI('Could not open session for environment <EnvId: {0}>, environment has deploying status.').format(environment_id))
    ...
    def get_result(self, request, environment_id, task_id):
        ...
        if result is not None:
            return result
        msg = (_('Result for task with environment_id: {} and task_id: {} was not found.').format(environment_id, task_id))
        LOG.error(msg)
